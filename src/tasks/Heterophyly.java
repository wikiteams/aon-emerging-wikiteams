...
public class Heterophyly {
	...
	public HashMap<Integer, Task> firstStep(Agent agent) {
		...
		for (Task singleTaskFromPool : tasks.values()) {
			...
			if(commonSkills == 0) {
				double proportion = (double)allAgentSkillsHetCl.size() / 0.1;
		...
		Double max = Collections.max(initial.keySet());
	...
	public HashMap<Integer, Task> secondStep(HashMap<Integer, Task> initial) {		
		...
		Integer max = Collections.max(initial.keySet());
	...
	public Task thirdStep(HashMap<Integer, Task> initial, Agent agent) {		
		...
		Double min = Collections.min(prepared.keySet());
